...
class TopoCoupledPointConvD_v2(nn.Module):
    """
    FiLM-based Topology Coupling (Ab3 style for comparison).
    
    Uses multiplicative FiLM modulation instead of residual low-rank adapter.
    No BatchNorm (similar to original Ab3 design).
    """
    def __init__(self, npoint, nsample, in_channel, out_channel, topo_dim=6,
                 weightnet=16, use_bn=True, use_leaky=True, residual_scale=1.0, rank=4):
        super(TopoCoupledPointConvD_v2, self).__init__()
        self.npoint = npoint
        self.nsample = nsample
        self.weightnet = WeightNet(3, weightnet)
        self.linear = nn.Linear(weightnet * in_channel, out_channel)
        
        # No BatchNorm for FiLM (Ab3 style)
        self.use_bn = False  # Force no BN
    ...
    def forward(self, xyz, points, topo_features):
        B = xyz.shape[0]
        N = xyz.shape[2]
        xyz = xyz.permute(0, 2, 1)
        points = points.permute(0, 2, 1)
        ...
        new_points = torch.matmul(
            input=new_points.permute(0, 1, 3, 2), 
            other=weights.permute(0, 3, 2, 1)
        ).view(B, self.npoint, -1)
        new_points = self.linear(new_points)
        new_points = new_points.permute(0, 2, 1)  # [B, out_channel, npoint]
        
        # FiLM modulation (multiplicative) - Ab3 style
        if topo_features is not None:
            t = self.film_gen(topo_features)  # [B, 64]
            gamma = self.gamma_gen(t).unsqueeze(-1)  # [B, out_channel, 1]
            beta = self.beta_gen(t).unsqueeze(-1)    # [B, out_channel, 1]
            
            # Multiplicative modulation (not residual!)
            new_points = (1 + gamma) * new_points + beta
